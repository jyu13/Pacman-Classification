...
def enhancedFeatureExtractorDigit(datum):
    ...
    ## DESCRIBE YOUR ENHANCED FEATURES HERE...
        use number of contiguous 0 = 2, 6 = 2, 8 = 3, 9 = 2 , rest = 1
    ##
    """
    ...
    def contiguousF(a,b):
        contiguous = []
        if a > 0:
            contiguous.append((a-1, b))
        if a +1< DIGIT_DATUM_WIDTH:
            contiguous.append((a+1, b))
        if b > 0:
            contiguous.append((a, b-1))
        if b +1< DIGIT_DATUM_HEIGHT:
            contiguous.append((a, b+1))
        return contiguous
    ...
    for x in range(DIGIT_DATUM_WIDTH):
        for y in range(DIGIT_DATUM_HEIGHT):
            features[(1,x,y)] = int(datum.getPixel(x,y) > datum.getPixel(x-1,y))
            features[(2,x,y)] = int(datum.getPixel(x,y) > datum.getPixel(x,y -1))

    contiguous = 0
    set_ = set()
    for x in range(DIGIT_DATUM_WIDTH):
        for y in range(DIGIT_DATUM_HEIGHT):
            if (x,y) not in set_ and datum.getPixel(x,y) <= 1 :
                contiguous += 1
                stack = [(x, y)]
                while stack:
                    p = stack.pop()
                    set_.add(p)
                    for c in contiguousF(*p):
                        if datum.getPixel(*c) <= 1 and c not in set_:
                            stack.append(c)
    features['c1'] = (contiguous >>1 )% 2
    features['c2'] = (contiguous >>1 )% 3
    features['c3'] = (contiguous >>1 )% 4
